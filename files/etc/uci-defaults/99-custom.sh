...
SETTINGS_FILE="/tmp/pppoe-settings"
...
if [ "$count" -eq 1 ]; then
   ...
   #wan_ifname=$(echo "$ifnames" | awk '{for(i=1;i<=NF;i++) if($i=="eth1") print $i}')
   # 剩余接口保留给LAN
   lan_ifnames=$(echo "$ifnames" | cut -d ' ' -f2-)
   #lan_ifnames=$(echo "$ifnames" | awk '{for(i=1;i<=NF;i++) if($i=="eth0") print $i}')
   # 设置WAN接口基础配置
   ...
   # 多网口设备 支持修改为别的ip地址
   uci set network.lan.ipaddr='192.168.89.2'
   uci set network.lan.netmask='255.255.255.0'
   uci set network.lan.gateway='192.168.89.1'
   uci add_list network.lan.dns='192.168.89.1'
   uci set network.lan.device="$lan_ifnames"
   uci set dhcp.lan.ignore='1'
   uci set dhcp.@dnsmasq[0].dns_redirect='0'
   echo "set 192.168.89.2 at $(date)" >>$LOGFILE
   # 判断是否启用 PPPoE
...
#NEW_DESCRIPTION="Compiled by ForestL"
